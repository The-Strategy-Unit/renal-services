...
1.  Download population estimates and future projections from ONS/Nomisweb, by age and gender. Get England, and NHS regions.

2.  Download CVD Prevent data - for each region.

3.  Apply Inside CKD to proportion split between S3 and S4-5, and calculate total prev and therefore undiagnosed from Inside CKD calculations. These are ASSUMPTIONS TO BE MADE CLEAR IN VALIDATION.

## The Process

### Population Data

Need population data for England, and NHS regions. Only includes 18 and over.
...
ckd_counts <- ckd_sys_time |> 
  rename(region = area_name,
         # sex = category_attribute,
         # age_band = metric_category_name,
         diag_s35 = numerator) |> 
  select(region, diag_s35, year, date, stella_time)

ckd_counts_eng <- ckd_counts |> 
  group_by(year, date, stella_time) |> 
  summarise(diag_s35 = sum(diag_s35)) |> 
  mutate(region = "England") |> 
  ungroup()
...
Using the baseline data from Inside CKD, split the diagnosed into stage 3 or stages 4/5. Then calculate total prevalence and counts of undiagnosed.
...
insideckd_loc <- "CKD - SD model/data/insideckd splits.xlsx"

insideckd_splits <- read_xlsx(insideckd_loc)

s3_pc <- insideckd_splits |> 
  filter(proportion == "diag S3 as pc S3-5") |> 
  pull(value)

s3_5_d <- insideckd_splits |> 
  filter(proportion == "S3-5 pc diagnosed") |> 
  pull(value)

s3_d <- insideckd_splits |> 
  filter(proportion == "S3 diag") |> 
  pull(value)

s45_d <- insideckd_splits |> 
  filter(proportion == "S4-5 diag") |> 
  pull(value)


# apply to CVDP counts
prev_counts <- ckd_counts |> 
  mutate(diag_s3 = diag_s35 * s3_pc,
         diag_s45 = diag_s35 - diag_s3,
         prev_s35 = diag_s35 / s3_5_d,
         prev_s3 = diag_s3 / s3_d,
         prev_s45 = diag_s45 / s45_d,
         undiag_s35 = prev_s35 - diag_s3,
         undiag_s3 = prev_s3 - diag_s3,
         undiag_s45 = prev_s45 - diag_s45) |> 
  
  relocate(diag_s35, .before = diag_s3) |> 
  
  pivot_longer(
    cols = diag_s35:undiag_s45,
...
saveWorkbook(wb, "ckd counts for stella array.xlsx", overwrite = TRUE)
```
