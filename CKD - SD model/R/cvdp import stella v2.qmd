...
1.  Download population estimates and future projections from ONS/Nomisweb, by age and gender. Get England, and NHS regions. ONS update estimates every summer (around July).

2.  Download CVD Prevent data - for each region - of prevalance (coded), uncoded and high-risk.

3.  Apply Inside CKD to proportion split between S3 and S4-5 for each of diagnosed and undiagnosed (uncoded).
...
query <- "https://www.nomisweb.co.uk/api/v01/dataset/NM_2006_1.data.csv?geography=2013265926,2013265924,2013265927,2013265921,2013265922,2013265928,2013265929,2013265925,2013265923&projected_year=2025...2043&gender=0&c_age=119,120,6...18,210&measures=20100&select=geography_name,geography_code,projected_year_name,gender_name,c_age_name,obs_value"
...
pop_data <- bind_rows(pop_est, pop_proj) |> 
  mutate( 
         stella_time = year + 0.5) # mid-year estimates
...
pop_stella <- pop_all |> 
  select(-year) |>
  mutate(region = paste0("ons pop[", region, "]")) |> 
...
ckd_sys_time <- map_df(avail_periods$time_period_id, ckd_ind_sys_fn) |> 
  ...
  mutate(year = parse_number(time_period_name),
        ...
        stella_time = as.numeric(yearqtr(date) + 0.25) #time starts at 0, Q4 of 2021
  ) |> 
...
cvd_mort <- map_df(area_list$area_id, ckd_mort_fn) |>
  left_join(area_list) |> 
  distinct() |>
  separate(time_period_name, into = c("start_month", "end_month"), sep = " - ") |> 
  mutate(
    start_date = dmy(paste("1", start_month)),
    end_date = ceiling_date(dmy(paste("1", end_month)), "month") - days(1),
    mid_date_m = month(ymd(start_date + (end_date - start_date) / 2)),
    month_dec = case_when(
      mid_date_m == 12 ~ 0.0,
      mid_date_m %in% c(3, 4) ~ 0.25,
      mid_date_m %in% c(6, 7) ~ 0.5,
      mid_date_m %in% c(9, 10) ~ 0.75
    ),
    rep_date = year(start_date) + month_dec
  ) |> 
  arrange(rep_date) |> 
  select(rep_date, area_name, rate = value)

stella_mort <- cvd_mort |> 
  mutate(stella_var = paste0("CVD all cause mortality p h th[", 
                             area_name, "]")) |> 
  select(-area_name) |> 
  pivot_wider(names_from = rep_date, values_from = rate) |> 
  arrange(stella_var)
...
Calculate variance for weightings used in calibration.
...
counts_var_wt <- prev_counts |> 
  ...
  pivot_wider(names_from = var_name, values_from = pop_var) 

mort_var_wt <- cvd_mort |> 
  group_by(region = area_name) |> 
  summarise("wt CVDP diag mort" = 1/(var(rate) *((n()-1)/n()))) |> 
  ungroup() 

var_wt <- left_join(counts_var_wt, mort_var_wt)
```
...
writeData(wb, "calib weights", var_wt)

saveWorkbook(wb, "ckd counts for stella v2.xlsx", overwrite = TRUE)
```
